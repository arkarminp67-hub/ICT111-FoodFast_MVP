...
We have selected a **Database-backed Web Application Stack** utilizing **Frontend web languages (HTML5, CSS3, JavaScript)** interacting with a central database infrastructure.
* **Justification:** This approach matches our team's current coding skill level and fits within our semester timeline while fully delivering the dynamic data management needed for live order routing, system state changes, and dashboard updates.

## 2. Component Breakdown
* **Frontend UI Platform:** Responsive, mobile-first Web Application built using HTML, CSS, and modern JavaScript.
* **Storage & Database:** Relational tables to process active data for vendors, menus, orders, and users.
